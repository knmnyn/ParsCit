...
if($isXmlInput){
  ...
  execute("$path/processOmniXML.pl -in $inFile -out $xmlInFile -xmlFeature");
  $inFile = $xmlInFile;
}

my $rXML = SectLabel::Controller::extractSection($inFile, $isXmlOutput, $modelFile, $configFile, $isXmlInput, $isDebug);
