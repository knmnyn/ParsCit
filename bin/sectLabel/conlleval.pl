...
### Thang 14/12/09: obtain confusion matrix
my $thang_count = 0;
my $isConfMatrix = 0;
my %confMatrix = ();
### End Thang 14/12/09: obtain confusion matrix

# sanity check
while (@ARGV and $ARGV[0] =~ /^-/) {
   if ($ARGV[0] eq "-l") { $latex = 1; shift(@ARGV); }
   elsif ($ARGV[0] eq "-r") { $raw = 1; shift(@ARGV); }
   elsif ($ARGV[0] eq "-c") { $isConfMatrix = 1; shift(@ARGV); } # Thang 14/12/09: obtain confusion matrix
   elsif ($ARGV[0] eq "-d") { 
      shift(@ARGV); 
...
### Thang 14/12/09: obtain confusion matrix
   if($isConfMatrix && $line ne ""){
     if(!$confMatrix{$correctType}){
       $confMatrix{$correctType} = ();
     }
     if(!$confMatrix{$correctType}->{$guessedType}){
       $confMatrix{$correctType}->{$guessedType} = ();
     }
     push(@{$confMatrix{$correctType}->{$guessedType}}, $line);
#     $confMatrix{$correctType}->{$guessedType} = $confMatrix{$correctType}->{$guessedType} ? ($confMatrix{$correctType}->{$guessedType} + 1) : 1;
     $thang_count++;
     if($thang_count == -1){
       last;
     }
...
### Thang 14/12/09: obtain confusion matrix
if($isConfMatrix){
  my @correctTypes = sort {$a cmp $b} keys %confMatrix;
  print STDERR "\n# Confusion matrix for exporting purpose\n";
  my %ignoreFields = ( 
		      "author" => 1,
		      "email" => 1,
		      "note" => 1,
  ...
  foreach my $correctType (@correctTypes){
    if($ignoreFields{$correctType}){ next; };
    print STDERR "|||$correctType";
  }
  print STDERR "\n";
  foreach my $correctType (@correctTypes){
    if($ignoreFields{$correctType}){ next; };
    ...
    foreach my $guessedType (@correctTypes){
      if($ignoreFields{$guessedType}){ next; };
      if($confMatrix{$correctType}->{$guessedType}){
	print STDERR "|||".scalar(@{$confMatrix{$correctType}->{$guessedType}});
      } else {
	print STDERR "|||0";
      }
    }
  ...
  print STDERR "\n# Confusion matrix for inspection purpose\n";
  foreach my $correctType (@correctTypes){
    print STDERR "$correctType:";
    my @guessedTypes = sort {$a cmp $b} keys %{$confMatrix{$correctType}};
    foreach my $guessedType (@guessedTypes){
      print STDERR " $guessedType-".scalar(@{$confMatrix{$correctType}->{$guessedType}});
    }
    print STDERR "\n";
  }
  print STDERR "\n";    
...
if (not $latex) {
   for $i (@sortedTypes) {
      ...
      my $TP = $correctChunk{$i};
      my $FN = $foundCorrect{$i} - $correctChunk{$i};
      my $FP = $foundGuessed{$i} - $correctChunk{$i};
      my $TN = $tokenCounter - $TP - $FN - $FP;
      my $accuracy = ($TP + $TN)/ $tokenCounter; 
      printf "FB1: %6.2f; TP=%d FN=%d FP=%d\n",$FB1, $TP, $FN, $FP;
