...
sub Help {
  ...
  print STDERR "       $progname [-q] keywordFile filename(s)...\n";
...
readDict($dictFile);
# keyword statistics
my $keywordFile = shift; #"/home/lmthang/RA/parscit/thang/keywords.txt";
...
#      my $isInDict = ($dictStatus != 0) ? "isInDict" : "no";
	...
	($word =~ /[\"\'\`][^s]?$/) ? "endQuote" :
      ...
      # Thang add: keyword features
      foreach(@tags){
	$tokens[$i] =~ s/^\p{P}+//g; #strip out leading punctuations
	$tokens[$i] =~ s/\p{P}+$//g; #strip out trailing punctuations
	$tokens[$i] =~ s/\d/0/g; #canocalize number into "0"

	if($keywords{$_}->{$tokens[$i]}){ #frequent keyword features
	  push(@{$feats[$j]}, "$_-$tokens[$i]");		    
	} else {
	  push(@{$feats[$j]}, "none");		    
	}
      }
...
sub readKeywordDict {
  ...
  print STDERR "Load keyword dict from $inFile\n";
